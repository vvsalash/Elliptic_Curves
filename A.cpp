...
std::vector<std::pair<UPmath::BigInteger, UPmath::BigInteger>> ElGamalEncrypt(UPmath::BigInteger& p, uint64_t P, const UPmath::BigInteger& g, const UPmath::BigInteger& k, const std::vector<uint64_t>& message) {
    std::vector<std::pair<UPmath::BigInteger, UPmath::BigInteger>> encryptedMessage;
    std::srand(time(NULL));
    for (int64_t value : message) {
        int64_t B = std::rand() % P;
        UPmath::BigInteger degree_of_generator = g.modPow(B, p);
        UPmath::BigInteger EncryptMessage = k.modPow(B, p);
        UPmath::BigInteger val(std::to_string(value));
        EncryptMessage = EncryptMessage * val;
        EncryptMessage %= p;
        encryptedMessage.emplace_back(degree_of_generator, EncryptMessage);
    }
...
int main() {
    ...
    UPmath::BigInteger p(P);
    UPmath::BigInteger g(G);
    UPmath::BigInteger k(K);

    std::vector<uint64_t> message;
    for (char symbol : sms) {
        message.push_back(CharToInt(symbol));
    }


    std::vector<uint64_t> group_message = Base64ToBaseP(message, P);
    std::vector<std::pair<UPmath::BigInteger, UPmath::BigInteger>> encrypted_messages = ElGamalEncrypt(p, P, g, k, group_message);
//    std::vector<std::pair<uint64_t, uint64_t>> encrypted_messages = ElGamalEncrypt(P, G, K, group_message);

//    for (const auto& encryption : encrypted_messages) {
//        std::cout << encryption.first << " " << encryption.second << "\n";
//    }
//
////
//
//
    for (const auto& encryption : encrypted_messages) {
        std::cout << encryption.first.toString() << " " << encryption.second.toString() << "\n";
    }
...
//        number = number * 10 + result[result.size() - 1 - i];
//    }
//    return number;
//}
...
//    uint64_t p;
//    std::cin >> p;
//    std::vector<std::vector<uint64_t>> mult_in_base_p;
//    mult_in_base_p.assign(p, std::vector<uint64_t>(p, 0));
//    for (uint64_t i = 0; i < p; ++i) {
//        for (uint64_t j = i; j < p; ++j) {
//            mult_in_base_p[i][j] = MultModP(i, j, p);
//        }
//    }
//    for (uint64_t j = 0; j < p; ++j) {
//        for (uint64_t i = j; i < p; ++i) {
//            mult_in_base_p[i][j] = mult_in_base_p[j][i];
//        }
//    }
//    for (int i = 1; i <= p; ++i) {
//        for (int j = 1; j <= p; ++j) {
//            std::cout << mult_in_base_p[i - 1][j - 1] << "\t";
//        }
//        std::cout << std::endl;
//    }
...
//    std::vector<uint64_t> message = {60, 36, 12};
//    uint64_t p = 83;
//    std::vector<uint64_t> mess = Base64ToBaseP(message, p);
//    for (size_t i = 0; i < mess.size(); ++i) {
//        std::cout << mess[mess.size() - 1 - i] << ' ';
//    }
//
//
