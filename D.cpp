...
public:
    ...
    std::vector<BigInt> GetCoefficients() {
    ...
    Poly operator-(Poly& other) const {
        ...
        if (result.Degree() < other.Degree()) {
            result.coefficients_.resize(other.coefficients_.size());
        }
        for (int64_t i = 0; i != static_cast<int64_t>(other.coefficients_.size()); ++i) {
...
Poly Power(Poly& poly, const BigInt& exponent, Poly& modulus) {
    Poly result(BigInt(1));
    Poly base = poly % modulus;
    BigInt exp = exponent;
...
Poly Inverse(const Poly& polynomial, const Poly& modulus) {
    Poly t(BigInt(0));
    Poly r = modulus;
    Poly new_t(BigInt(1));
    Poly new_r = polynomial;
    Poly quotient, r_temporary, t_temporary;

    while (new_r != Poly(BigInt(0))) {
        Poly q = r / new_r;
        quotient = q;

        r_temporary = quotient * new_r;
        t_temporary = quotient * new_t;

        r_temporary = r - r_temporary;
        t_temporary = t - t_temporary;
    ...
    Poly res = (t * polynomial) % modulus;
    BigInt a = res.GetCoefficients().back();
    BigInt inverse = Power(a, p - 2, p);
    std::vector<BigInt> coeffs = t.GetCoefficients();
    for (auto& i : coeffs) {
        i = (i * inverse) % p;
    }
    Poly result(coeffs);
...
int main() {
    ...
    Poly F(f_polynomial);
    BigInt n = static_cast<int64_t>(f_polynomial.size()) - 1;
    ...
    while(getline(std::cin, X)) {
        getline(std::cin, Y);
        std::vector<BigInt> x_poly;
        std::vector<BigInt> y_poly;
        StringToCoefficients(X, x_poly);
        StringToCoefficients(Y, y_poly);
        Poly x(x_poly);
        Poly y(y_poly);
        x = Power(x, k, F);
        x = Inverse(x, F);
        Poly mess = (y * x) % F;
        std::vector<BigInt> numbers = mess.GetCoefficients();
        BigInt size = static_cast<int64_t>(numbers.size());
        for (auto& i : numbers) {
            message.push_back(i);
        }
        while (size < n) {
            message.push_back(BigInt(0));
            size += 1;
        }
    }
