...
int main() {
    ...
    std::string public_x, public_y;
    std::cin >> public_x >> public_y;
    Point public_key(public_x, public_y);
    int n;
    std::cin >> n;
    while (n--) {
        std::string message;
        std::cin >> message;
        std::vector<int> number = StringToVector(message);
        Modular current_x = ConvertFromPSystem(number, 64);
        Modular current_y = Sqrt(current_x * current_x * current_x + coefficient_a * current_x + coefficient_b);
        Point current(current_x, current_y);
        int degree = 1;
        std::cout << Power(g, degree) << "\n";
        std::cout << current * Power(public_key, degree) << "\n";
    }
    return 0;
